...
/*-------------------------------------------------
* Separate [Include] Definition Start↓
* C++ のヘッダーファイルをインクルード
-------------------------------------------------*/
#include <iostream>     /*C++におけるストリームヘッダー*/
#include <string>       /*basic_string tmp(typedef)文字列を扱う際に利用できる*/
#include <functional>   /*ラムダ式や関数オブジェクトを使うときに*/

using namespace std;    /*標準名前空間の省略宣言*/
/*-------------------------------------------------
* Separate [Include] Definition End↑
* C++ のヘッダーファイルをインクルード
-------------------------------------------------*/


/*-------------------------------------------------
* Separate [Constant] Definition Start↓
* //定数を定義する場所
-------------------------------------------------*/
#define SPELL_COST  (3) //呪文の消費MPを定義する
/*-------------------------------------------------
* Separate [Constant] Definition End↑
* //定数を定義する場所
-------------------------------------------------*/


/*-------------------------------------------------
* Separate [Enumeration] Definition Start↓
* //列挙定数を定義する場所
-------------------------------------------------*/
//モンスターの種類を定義する
enum
...
/*-------------------------------------------------
* Separate [Enumeration] Definition End↑
* //列挙定数を定義する場所
-------------------------------------------------*/
...
/*-------------------------------------------------
* Separate [Structure] Definition Start↓
* //構造体を宣言する場所
* クラスの宣言及び定義
-------------------------------------------------*/
/*Log 11/7:松本*/
/*-------------------------------------------------
typedef struct → classに
クラス名は構造体のものをそのまま利用している
-------------------------------------------------*/
//キャラクターの構造体を宣言する
class CHARACTER
{
...
/*-------------------------------------------------
* Separate [Structure] Definition End↑
* //構造体を宣言する場所
* クラスの宣言及び定義
-------------------------------------------------*/
...
/*-------------------------------------------------
* Separate [DebugVariable] Definition Start↓
* デバッグ用テスト変数
-------------------------------------------------*/
char inputString;
...
/*-------------------------------------------------
* Separate [Variable] Definition Start↓
* //変数を宣言する場所
-------------------------------------------------*/
//モンスターのステータスの配列を宣言する           

/*キャラクタークラスのインスタンス化(コンストラクタ実行)*/
//monsters = new CHARACTER[MONSTER_MAX];

/*コンストラクタで代入された値を修正し、再設定*/
...
/*-------------------------------------------------
* Separate [Variable] Definition End↑
* //変数を宣言する場所
-------------------------------------------------*/
...
/*-------------------------------------------------
* Separate [Functions] Definition Start↓
* //関数を宣言する場所
-------------------------------------------------*/
//ゲームを初期化する関数を宣言する
void Init()
...
void SelectCommand()
{
    ...
    while (1)
    {
        ...
        /*11/7:松本:テスト中*/
        /*-------------------------------------------------
        * ImplementationTest [InputGetKey] Definition Start↓
        * 目的：iostreamを利用したキー入力への変更
        * 進捗：テストプログラム作成中：cinで代入される値の分析
        -------------------------------------------------*/
...
/*-------------------------------------------------
* Separate [Functions] Definition End↑
* //関数を宣言する場所
-------------------------------------------------*/

/*-------------------------------------------------
* Separate [Execution] Definition Start↓
* //プログラムの実行開始点を宣言する
-------------------------------------------------*/
int main()
{
...
/*-------------------------------------------------
* Separate [Execution] Definition End↑
* //プログラムの実行開始点を宣言する
-------------------------------------------------*/
